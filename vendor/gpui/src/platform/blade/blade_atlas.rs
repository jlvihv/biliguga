...
impl PlatformAtlas for BladeAtlas {
    ...
    fn remove(&self, key: &AtlasKey) {
        ...
        if let Some(mut texture) = texture_slot.take() {
            // `drop_image` must return the tile to the atlas allocator. Without
            // this, every new RenderImage consumes another region forever. This
            // is especially visible for software-rendered video frames, which
            // create a new RenderImage for every frame.
