...
impl RenderImage {
    ...
    /// Consumes a single-frame image and returns its underlying pixel buffer.
    /// This lets software video renderers recycle large frame allocations once
    /// the image has been removed from the platform atlas.
    pub fn into_single_frame_bytes(mut self) -> Option<Vec<u8>> {
        if self.data.len() != 1 {
            return None;
        }
        self.data
